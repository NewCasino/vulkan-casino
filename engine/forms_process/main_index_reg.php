...
if(!defined('CASINOENGINE')) { die("Нет доступа!<script>location.href='/';</script>"); }
	//Читаем post массив
		...
		$регистрация_пароль_фильтер = preg_match('/^[A-Za-z0-9]{1,20}$/', $регистрация_пароль);
			...
			if ($регистрация_email == '') 				 			{ $error .= 'Не введён почтовый адрес<br>';} else
		{
		  if ($регистрация_email_фильтер == false) 			{ $error .= 'Не корректный почтовый адрес<br>';} }
		  	if (!$confirm) {
		$error .= 'Вы не согласились с условиями казино';
	}
	...
	$confirm
		...
		//Тут надо проверить на то не существует ли уже пароля перед тем как писать в базу.
...
@mysql_query("INSERT INTO clients
		...
		//Отошлем письмо о регистрации
			...
			if (isAjaxRequest()) {
				unset($pid);
				exit(json_encode(array('status' => 0, 'message' => '', 'url' => '/' . $_SESSION['language'] . '/')));
			}
			
			echo '<script>location.href="/'.$_SESSION['language'].'/";</script>';
			unset($pid);
			die() or exit();
	...
	if ($регистрация_логин == '' and $регистрация_пароль == ''  and $регистрация_email == '')
	{ $error = ''; }

	include(ENGINE_DIR."forms/main_index_reg.php");
?>