...
$title = "Панель администратора - Рассылка ПМ";
$menu = "module";
...
{
    $error .= "Не выбранн пользователь для рассылки!<br>";
}
if ( $режим == "send" && $error == "" )
{
    if ( $send_mode == "pm_user" && $рассылка_логин != "" && ( $рассылка_логин_фильтер = true ) && $рассылка_тема != "" && $рассылка_сообщение != "" )
    {
        ...
        if ( $клиенты_запрос != "" || $клиенты_запрос != "0" )
        {
            ...
            echo "<script>alert(\"Личное сообщение для клиента: ".$рассылка_логин." успешно отправленно!\");</script>";
    ...
    if ( $send_mode == "pm_all" && $рассылка_тема != "" && $рассылка_сообщение != "" )
    {
        ...
        echo "<script>alert(\"Личные сообщения для клиентов успешно отправленны!\");</script>";
        echo "<script>location.href=\"/acp/acp_admin/admin_mass_pm.php\";</script>";
        exit( );
        if ( !TRUE )
        {
