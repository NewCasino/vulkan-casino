	...
	case 'edit_freekassa':
		...
		if (!$error) {
		    if ($_SESSION['session_admin'] != $demo) {
		        ...
		        echo "<script>alert(\"Данные настроек FreeKassa обновлены!\");</script>";
		    } else echo "<script>alert(\"Вы не можете редактировать настройки FreeKassa в демо режиме!\");history.back();</script>";
		}
		if ($form_mrh_login == "" && $form_mrh_pass1 == "" && $form_mrh_pass2 == "") $error = "";
	break;
